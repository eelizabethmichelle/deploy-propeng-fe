...
export async function GET(request: NextRequest) {
  ...
  if (!API_BASE_URL) {
    ...
    return NextResponse.json({ message: "API base URL configuration not found." }, { status: 500 })
  }

  // Construct the backend URL for the monthly detail endpoint
  const backendUrl = `${API_BASE_URL}/api/absen/kelas/${kelasId}/monthly-detail/?month=${month}`
  ...
  try {
    ...
    return NextResponse.json({ message: "Internal server error while contacting backend." }, { status: 500 })
  }
}
