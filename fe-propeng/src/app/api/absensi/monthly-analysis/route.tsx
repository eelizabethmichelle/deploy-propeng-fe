...
export async function GET(request: NextRequest) {
  ...
  // Forward month param to backend
  let backendUrl = `${API_BASE_URL}/api/absen/kelas/${kelasId}/monthly-analysis/`;
  if (month) backendUrl += `?month=${month}`;

  console.log(`[API Route /api/absensi/monthly-analysis] Fetching from: ${backendUrl}`);
  ...
  try {
    ...
    if (!backendResponse.ok) {
      ...
      return NextResponse.json(
        { message: errorData.message || "Failed to fetch monthly attendance analysis." },
