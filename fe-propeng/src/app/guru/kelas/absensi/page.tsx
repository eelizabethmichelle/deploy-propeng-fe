  ...
  const getAttendanceCell = (status: 'Hadir' | 'Sakit' | 'Izin' | 'Alfa' | null, date: string, studentId: number) => {
    ...
    if (!status) {
      return (
        ...
        >
          <div className="text-sm font-medium text-[#041765] bg-[#F7F8FF] py-0.5 px-2 rounded-md w-full text-center">{displayDate}</div>
        </div>
      );
    }
  ...
  const formatDateHeader = (dateString: string) => {
    ...
    return (
      <div className="flex flex-col items-center gap-0 w-full h-full">
        <div className="text-sm font-medium text-[#041765] bg-[#F7F8FF] py-0.05 px-2 rounded-md w-full text-center">{displayDate}</div>
        <MousePointerClick 
          size={20} 
